...
export function buildCreateBody(
  ...
  if (!flags.schedule)
    throw new ValidationError(
      ...
      "Add --schedule periodic or --schedule scheduled",
    );
  if (!flags.type)
    throw new ValidationError(
      "--type is required",
      "Add --type script or --type agent",
    );
  if (!flags.channel)
    throw new ValidationError(
      "--channel is required",
      "Add --channel <slack-channel-id>",
    );

  // Cross-validate execution flags
  if (flags.type === "script" && flags.prompt) {
    throw new ValidationError(
      "--prompt cannot be used with --type script",
      "Remove --prompt or change to --type agent",
    );
  }
  if (flags.type === "agent" && flags.script) {
    throw new ValidationError(
      "--script cannot be used with --type agent",
      "Remove --script or change to --type script",
    );
  }
  if (flags.type === "script" && !flags.script) {
    throw new ValidationError(
      "--script is required for script jobs",
      'Add --script "<your script content>"',
    );
  }
  if (flags.type === "agent" && !flags.prompt) {
    throw new ValidationError(
      "--prompt is required for agent jobs",
      'Add --prompt "<what the agent should do>"',
    );
  }
  ...
  if (flags.schedule === "periodic") {
    if (!flags.seconds)
      throw new ValidationError(
        "--seconds is required for periodic schedule",
        "Add --seconds <N>",
      );
    ...
    throw new ValidationError(
      `Invalid schedule type: ${flags.schedule}`,
      "Use --schedule periodic or --schedule scheduled",
    );
  }
...
const createCommand = defineCommand({
  ...
  args: {
    ...
    schedule: {
      type: "string",
      description: "Schedule type: periodic | scheduled",
    },
    ...
    weekday: {
      type: "string",
      description: "Weekday 0-6, Sunday=0 (scheduled)",
    },
    ...
    json: {
      type: "string",
      description: "Raw JSON body (overrides other flags)",
    },
  },
  async run({ args }) {
