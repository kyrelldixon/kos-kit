...
import { type CLIResponse, error, output, success } from "../lib/output";
...
export async function handleList(client: ApiClient): Promise<CLIResponse> {
  ...
  if (res.status !== 200) {
    return error(
      ...
      "Check the server logs or run: kos doctor",
      [],
    );
  }
  return success("kos config list", res.data, [
    {
      command: "kos config set <key> <value>",
      description: "Update a config value",
    },
    {
      command: "kos config get <key>",
...
export async function handleGet(
  ...
  if (value === undefined) {
    return error(
      ...
      "Run kos config list to see available keys",
...
export async function handleSet(
  ...
  return success(`kos config set ${key}`, res.data, [
    ...
    {
      command: `kos config get ${key}`,
      description: `Verify ${key} was updated`,
    },
  ]);
}
