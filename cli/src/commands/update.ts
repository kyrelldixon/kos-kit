...
export const updateCommand = defineCommand({
  ...
  async run() {
    ...
    const oldHead = (await $`git -C ${kosDir} rev-parse HEAD`.quiet())
      .text()
      .trim();
    ...
    const newHead = (await $`git -C ${kosDir} rev-parse HEAD`.quiet())
      .text()
      .trim();
    ...
    if (oldHead === newHead) {
      ...
      const log = (
        await $`git -C ${kosDir} log --oneline ${oldHead}..${newHead}`.quiet()
      )
        .text()
        .trim();
