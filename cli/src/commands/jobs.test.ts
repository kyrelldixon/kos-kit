...
describe("buildCreateBody", () => {
  ...
  test("script with newlines survives flag round trip", () => {
    ...
    if (typeof exec !== "object" || exec === null || !("script" in exec)) {
      throw new Error("expected execution.script");
    }
    expect(exec.script).toBe("#!/bin/bash\necho hello\necho world");
  });
...
describe("buildCreateBody --json mode", () => {
  test("parses raw JSON and injects name", () => {
    const json =
      '{"schedule":{"type":"periodic","seconds":60},"execution":{"type":"script","script":"echo hi"},"destination":{"chatId":"C123"}}';
  ...
  test("positional name overrides name in JSON", () => {
    const json =
      '{"name":"wrong","schedule":{"type":"periodic","seconds":60},"execution":{"type":"script","script":"echo hi"},"destination":{"chatId":"C123"}}';
